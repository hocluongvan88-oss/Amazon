...
| 23 | `024_ops_cockpit.sql` | **Phase 3 – Cockpit vận hành**: `ops_cockpit(t, asof)` 9 nhóm (đơn hàng, tồn kho, trả hàng, từ khoá, traffic, chuyển đổi, khuyến mãi, review, quảng cáo) với badge tươi/quá hạn/thiếu (`feed_badge`), KPI 1/7/30 ngày + baseline 30 ngày trước (thiếu → NULL, không phải 0); `cockpit_actions(t)` hàng đợi P0–P3 có evidence/owner/deadline/link; `cockpit_open_task()` mở task có dedupe; `policy_register.cockpit_thresholds`. Test: `tests/024_ops_cockpit_test.sql` | Bắt buộc |
