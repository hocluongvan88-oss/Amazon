...
BEGIN
  ...
  INSERT INTO public.amazon_skus (tenant_id, asin, sku, title, marketplace, current_price, cogs, fee_per_unit, referral_fee_pct)
    VALUES (tn, 'B0TEST00001', 'SKU-1', 'Test 1', 'US', 25, 8, 4, 15) RETURNING id INTO k1;
  INSERT INTO public.amazon_skus (tenant_id, asin, sku, title, marketplace, current_price, cogs, fee_per_unit, referral_fee_pct)
    VALUES (tn, 'B0TEST00002', 'SKU-2', 'Test 2', 'US', 30, 10, 5, 15) RETURNING id INTO k2;
  ...
  r := public.ingest_normalize_row('orders', '{"order_id":"111-1","order_date":"bad","asin":"b0test00001","quantity":"2"}'::jsonb);
  IF (r->>'_ok')::boolean = false AND r->>'_err' LIKE '%order_date%' AND r->>'asin' = 'B0TEST00001'
  THEN passes := passes + 1; ELSE fails := array_append(fails, 'normalize marks bad date, uppercases asin: ' || r::text); END IF;
  r := public.ingest_normalize_row('orders', '{"order_id":"111-1","asin":"B0TEST00001","quantity":"2"}'::jsonb);
  ...
  r := public.ingest_add_rows(bid, jsonb_build_array(
    jsonb_build_object('order_id','111-0001','order_date',d0::text,'asin','B0TEST00001','sku','SKU-1','quantity','2','item_sales','49.98','status','Shipped'),
    jsonb_build_object('order_id','111-0002','order_date',d0::text,'asin','B0TEST00001','sku','SKU-1','quantity','1','item_sales','24.99','status','Cancelled'),
    jsonb_build_object('order_id','111-0003','order_date',d1::text,'asin','B0TEST00002','sku','SKU-2','quantity','3','item_sales','90','status','Shipped'),
    jsonb_build_object('order_id','111-0004','order_date',d1::text,'asin','B0TEST00009','sku','SKU-9','quantity','1','item_sales','10','status','Shipped'),
    jsonb_build_object('order_id','111-0005','order_date','31/31/2026','asin','B0TEST00001','sku','SKU-1','quantity','1','item_sales','10','status','Shipped')
  ));
  ...
  IF (r->>'rows_invalid')::int = 1 AND (r->>'unknown_asin_count')::int = 1 AND r->'unknown_asins'->>0 = 'B0TEST00009'
  ...
  PERFORM public.ingest_add_rows(bid2, jsonb_build_array(
    jsonb_build_object('order_id','111-0001','order_date',d0::text,'asin','B0TEST00001','sku','SKU-1','quantity','2','item_sales','49.98','status','Shipped'),
    jsonb_build_object('order_id','111-0003','order_date',d1::text,'asin','B0TEST00002','sku','SKU-2','quantity','5','item_sales','150','status','Shipped')));
  ...
  PERFORM public.ingest_add_rows(bid, jsonb_build_array(
    jsonb_build_object('return_date',d1::text,'order_id','111-0003','asin','B0TEST00002','quantity','1','reason','DEFECTIVE','refund_amount','30'),
    jsonb_build_object('return_date',d1::text,'order_id','111-0001','asin','B0TEST00001','quantity','1','reason','NOT_AS_DESCRIBED','refund_amount','24.99','customer_comment','Kích thước nhỏ hơn mô tả')));
  ...
  bid := (r->>'batch_id')::uuid;
  PERFORM public.ingest_add_rows(bid, jsonb_build_array(
    jsonb_build_object('date',d0::text,'asin','B0TEST00001','campaign','SP-Auto','ad_group','AG1','impressions','4,210','clicks','58','spend','$31.20','sales','224.91','orders','9'),
  ...
  bid := (r->>'batch_id')::uuid;
  PERFORM public.ingest_add_rows(bid, jsonb_build_array(
    jsonb_build_object('date',d0::text,'asin','B0TEST00001','sessions','120','page_views','150','units_ordered','99','ordered_product_sales','999','buy_box_pct','98%','unit_session_pct','0.05')));
  r := public.ingest_commit(bid);
  SELECT sessions, units INTO n, n2 FROM public.sku_daily_snapshots WHERE sku_id = k1 AND date = d0;
  SELECT unit_session_pct INTO nn FROM public.traffic_daily WHERE tenant_id = tn AND asin = 'B0TEST00001';
  ...
  bid := (r->>'batch_id')::uuid;
  PERFORM public.ingest_add_rows(bid, jsonb_build_array(
    jsonb_build_object('date',d1::text,'asin','B0TEST00001','sku','SKU-1','fc','PHX7','available','40','inbound','10'),
    jsonb_build_object('date',d1::text,'asin','B0TEST00001','sku','SKU-1','fc','ONT8','available','25','inbound','0')));
  r := public.ingest_commit(bid);
  ...
  bid := (r->>'batch_id')::uuid;
  PERFORM public.ingest_add_rows(bid, jsonb_build_array(
    jsonb_build_object('promo_id','CPN-1','promo_type','Coupon','name','10% off','asins','B0TEST00001; b0test00002','start_at',d0::text,'end_at',(d0+7)::text,'discount_type','percent','discount_value','10'),
    jsonb_build_object('promo_id','LD-1','promo_type','Lightning Deal','asins','B0TEST00001','start_at',d1::text,'end_at',(d1-1)::text)));
  r := public.ingest_dry_run(bid);
